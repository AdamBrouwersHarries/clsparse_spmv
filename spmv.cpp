...
int main (int argc, char* argv[])
{
    ...
    if (argc > 2){
        std::cerr << "Reading # of iterations from second command line argument." << std::endl;
...
//                                    0, y.num_values * sizeof(float), 0, nullptr, nullptr);
    ...
    /** Perform 30 spmv calls - they should be fast, and this should get statistical accuracy... */
    for(int i = 0;i<iterations;i++){
        status = clsparseScsrmv(&alpha, &A, &x, &beta, &y, control);
        if (status != clsparseSuccess)
        {
            std::cerr << "Problem with execution SpMV algorithm."
                      << " Error: " << status << " = " << stringifyStatus(status) << std::endl;
        }
        
        cl_event spmvEv;
        clsparseGetEvent(control, &spmvEv);
        ...
        clGetEventProfilingInfo(spmvEv, CL_PROFILING_COMMAND_START, sizeof(time_start), &time_start, NULL);
        clGetEventProfilingInfo(spmvEv, CL_PROFILING_COMMAND_END, sizeof(time_end), &time_end, NULL);
    ...
    if (status != clsparseSuccess)
    {
        std::cerr << "Problem with releasing control object."
                  << " Error: " << status << std::endl;
    }
    ...
    if (status != clsparseSuccess)
    {
        std::cerr << "Problem with closing clSPARSE library."
                  << " Error: " << status << std::endl;
    }
